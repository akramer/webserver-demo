use axum::{http::header::HeaderMap, routing::get, Router, extract::Path, extract::State};
...
async fn main() {
    ...
    let state = AppState{cpu_percent: Arc::new(Mutex::new(0_f64))};
    let app = Router::new()
        // `GET /` goes to `root`
        .route("/", get(root))
        .route("/set_cpu_percent/:percent", get(set_cpu_percent).with_state(state.clone()));
